...
const TaskInfo = ({ columnId, ...task }: TaskView) => {
  const numSubTasksDone = task.subtasks
    ? task.subtasks.filter((subtask: SubTaskType) => subtask.is_done == true)
        .length
    : 0;
  ...
  const [showModalDeleteTask, openModalDeleteTask, closeModalDeleteTask] =
    useModal();
  ...
  const changeSubTaskIsDone = useBoardDetail(
    (state) => state.changeSubTaskIsDone
  );
  const OnChange = (e) => {
    ...
    <>
      <div className="task-info-container">
        <div className="task-title-container">
          <h2 className="text-theme">{task.title}</h2>
        ...
        <div className="subtasks-container">
          <h5 className="text-theme subtask-info">
            Subtasks ( {numSubTasksDone} of {totalSubTasks})
          </h5>
      ...
      <Modal showModal={showModalEditTask} closeModal={closeModalEditTask}>
        <TaskForm
          value={task}
          columnId={columnId}
      ...
      <Modal showModal={showModalDeleteTask} closeModal={closeModalDeleteTask}>
        <DeleteDialog
          type="task"
          name={task.title}
          onDelete={OnDelete}
